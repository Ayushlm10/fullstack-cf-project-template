...
async function main() {
	...
	if (baseDomain) {
		...
		log(`   Worker: Private (RPC-only)${usePublicApi ? ` + Public API at ${apiDomain}` : ''}`, colors.dim);
	} else {
	...
	if (baseDomain) {
		...
		log(`  6. Deploy: Web will be at ${webDomain}${usePublicApi ? `, API at ${apiDomain}` : ''}`, colors.dim);
